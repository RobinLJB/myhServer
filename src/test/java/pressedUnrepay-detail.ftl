	...
	<table class="table detail-info">
			...
			<tr>
				...
				<td>
					...
					<#elseif member.memberStatus ==2>
						<span class="label label-warning">争议可以</span>
				...
				<tr>
					<th>身份证照片A：</th>
					<td>
						<#if cardMap.card_imgurl??>
						<ul id="loanImgs">
							<li><img src="${cardMap.card_imgurl!} " /></li>
						</ul>
						<#else>
					...
					<th>身份证照片B：</th>
					<td>
						<#if cardMap.people_imgurl??>
						<ul id="loanImgs">
							<li><img src="${cardMap.people_imgurl!} " /></li>
						</ul>
						<#else>
						<span class="label label-warning">无图片</span>
						</#if>
		...
		<table class="table detail-info">
			...
			<tbody>
				...
				<tr>
					<th>芝麻信用</th>
					<td>${phoneAndAplipayMap.alipay_score!}</td>
					<th>认证时间</th>
					<td>${phoneAndAplipayMap.alipay_audit_time!}</td>
				</tr>
				<tr>
					<th>手机使用天数</th>
					<td>${phoneAndAplipayMap.phone_use_month!}</td>
					<th>认证时间</th>
					<td>${phoneAndAplipayMap.phone_audit_time!}</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="table detail-info">
			...
			<thead>
       		...
       		<tbody>
        		<tr>
          			<#if pressList??>
						<#list pressList as lists>
							<tr>
								<td >${lists_index+1}</td>
								<td >${lists.phoneCall!0}</td>
								<td>${lists.repayStatus!}</td>
								<td>${lists.pressTime!}</td>
								<td>${lists.review!}</td>
							</tr>
						</#list>
					<#else>
						<tr>
							<th colspan="4">没有催收记录</th>
						</tr>
					</#if>
        		</tr>
       		</tbody>
