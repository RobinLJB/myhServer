...
<div class="page-content">
	...
	<form class="form-horizontal" id="loanform"  style="margin-top:20px;" action="${context}/admin/loan/add.do"  method="post">
		...
		<#if loanMap.status=="2"||loanMap.status=="1">
			<button   type="submit" class="btn btn-primary btn-lg"   style="margin-left: 141px;">发布借款</button>
			<button type="button" onclick="audit(3)" class="btn btn-danger" data-loading-text="正在保存..." autocomplete="off" style="margin-left: 141px;">审核不通过</button>
			<#elseif loanMap.status=="3"    >
			<button   type="button" class="btn btn-danger btn-lg"  style="margin-left: 141px;">审核不通过</button>
			<#elseif loanMap.status=="4"    >
			<!--<button   type="button" class="btn btn-primary btn-lg"  style="margin-left: 141px;">已成功发布借款</button>-->
		</#if>
		<input type="hidden" name="applicationId" id="applicationId" value="${loanMap.id!}">
		<input type="hidden" name="uid" id="uid" value="${loanMap.uid!}">
	</form>	
</div>	
</@override>
