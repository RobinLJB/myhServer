...
<div class="page-content">
	<div class="page-bar">
		<ul class="page-breadcrumb">
			...
			<li>
				<a href="${context}/admin/loan/index.html">全部借款</a>
				<i class="fa fa-angle-right"></i>
			</li>
			<li>
				<a href="#">借款详情</a>
	...
	<div id="stepItems">
		<ul class="processor_bar grid_line">
			...
			<li data-id="4" class="step grid_item size1of5">
				<h4>4 还款中</h4>
			</li>
			<li data-id="5" class="no_extra step grid_item size1of5">
				<h4>5 已完成</h4>
			</li>
		</ul>
	</div>
	...
	<table class="table detail-info">
			...
			<tbody>
				...
				<tr>
					...
					<td class="blue12 left">
						${loan.min_invest_amount}元
					</td>
					<th>最高投标金额：</th>
					<td class="f66 leftp200">
						${loan.max_invest_amount}元                                         
					</td>
				</tr>
				<tr>
					<th>投标奖励:</th>
					<td>
						<#if loan.excitation_type == "1">
							无
						<#elseif loan.excitation_type == "2">
							${loan.excitation_sum}元
						<#elseif loan.excitation_type == "3">
							${loan.excitation_sum}%
						</#if>
					</td>
					<th>投标密码：</th>
				...
				<tr>
					...
					<td colspan="3">
						...
						<ul id="loanImgs">
							...
							<li><img style="width: 200px;height:150px;" src="${img}" /></li>
							</#list>
						</ul>
						</#if>
					</td>
		...
		<button class="btn btn-success" type="button" onclick="audit(1)">复审通过</button>
		<button class="btn btn-danger" type="button" onclick="cancel()">流标</button>
		</#if>
</div>
</@override>
...
<script>
	...
	function audit(status){
		...
		param['status'] = status;
		$.post('${context}/admin/loan/firstaudit.do',param,function(resp){
			if(resp.code == 0){
				spark.notify(resp.message);
				spark.redirect('${context}/admin/loan/'+${loan.id}+'.html');
			} else{
	...
	function cancel(){
		spark.confirm("流标后无法撤销，确定继续吗？",function(){
			var param = {};
			param['id'] = ${loan.id};
			$.post('${context}/admin/loan/cancel.do',param,function(resp){
				if(resp.code == 0){
					spark.notify(resp.message);
					spark.redirect('${context}/admin/loan/'+${loan.id}+'.html');
				} else{
	...
	function audit(status){
		...
		param['opinion'] = $('#auditOpinion').val();
		param['status'] = status;
		$.post('${context}/admin/loan/fullScaleAudit.do',param,function(resp){
			if(resp.code == 0){
				spark.notify(resp.message);
				spark.redirect('${context}/admin/loan/'+${loan.id}+'.html');
			} else{
