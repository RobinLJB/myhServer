...
<div class="page-content">
    ...
    <button class="btn btn-danger" type="button" onclick="audit()">确认还款</button>
...
<script>
    ...
    function audit() {
        ...
        if (realamount == '' || isNaN(new Number(realamount))) {
            alert("请输入数字");
            $('#amount').focus();
            return;
        }
       ...
       /* param['managerFee'] = managerFee;没有管理费，只有逾期费*/
        $.ajax({
            url:'/admin/repay/aaaaa',
            data:param,
