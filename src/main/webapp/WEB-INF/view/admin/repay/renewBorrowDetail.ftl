...
<#--<div class="alert alert-danger" style="margin-top:20px;">
    ...
    <table class="table detail-info">
        <thead>
        <th colspan="4">续期信息</th>
        </thead>
        <tbody>
        <tr class="active">
            <th>续期天数</th>
            <td id="renewDays" value=""/>7天</td>
            <th>续期费用</th>
            <td id="renewFee" value="${borrowMap.serviceFee!}">${borrowMap.serviceFee!}元</td>
        </tr>
        </tbody>
    </table>
    <button class="btn btn-danger" type="button" onclick="audit(10)">确认续期</button>
</div>
</@override>
...
<script>
    ...
    function audit(status) {
    	var cmsg=status==10?'续期':'';
    	if(confirm('确定要'+cmsg+'?')){
		    var param = {};
		    param['id'] = ${borrowMap.id};
		    param['renewDays'] = 7;
		    param['renewFee'] = ${borrowMap.serviceFee!};
		    $.post('${context}/admin/repay/renew/borrow.do', param, function (resp) {
		        if (resp.code == 0) {
