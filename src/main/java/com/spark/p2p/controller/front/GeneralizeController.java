...
public class GeneralizeController extends FrontController {
    ...
    public @ResponseBody
    MessageResult getVerificationCode() throws UnsupportedEncodingException {
        ...
        if (phone.equals("18655553616") || phone.equals("13399558122")) {
            ...
            ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getSession().setAttribute("VERIFY_PHONE_CODE_" + phone, randomCode);
            return null;
        } else {
    ...
    @RequestMapping(value={"verification/codeNew"}, method={org.springframework.web.bind.annotation.RequestMethod.POST})
    @ResponseBody
    public MessageResult getVerificationCodeNew(HttpServletRequest request)
      throws UnsupportedEncodingException
    {
      ...
      if ((this.loginRequest.get(ip) != null) && (new Date().getTime() - ((Long)this.loginRequest.get(ip)).longValue() < 60000L)) {
    ...
    public @ResponseBody
    MessageResult verify(HttpServletRequest request) throws Exception {
        String phone = request("phone");
        ...
        String reruestType = request("reruestType");
        String requestCode = request("requestCode");
        String sessionCode = (String) ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getSession().getAttribute("VERIFY_PHONE_CODE_" + phone);
    ...
    public void generalizeMobileType() throws Exception {
        String requestCode = request("requestCode");
        ...
        if (memberMap != null) {
            ...
            if (memberId != null) {
                Map<String, String> ralationMap = new Model("member_ralation").where("afterMemberId=?", memberId).find();
                if (ralationMap != null) {
