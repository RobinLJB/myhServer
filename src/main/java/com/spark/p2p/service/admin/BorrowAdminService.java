...
public class BorrowAdminService extends BaseService {
    ...
    public DataTable queryForFristAudit(DataTableRequest params, long roleid) throws SQLException {
        ...
        /*if(StringUtils.isEmpty(time)) {
        	// 如果当前时间为空默认查询 七日数据 避免数据库查询速度慢 必须要将fristSubmitTime也重放一遍避免为空 后边不走条件
        	params.addColumn("fristSubmitTime",DateUtil.getDate());
        	params.addColumn("min_fristSubmitTime",  DateUtil.YYYY_MM_DD.format(DateUtil.dateAddDay(new Date(), -1)) + " 00:00:00");
            params.addColumn("max_fristSubmitTime",DateUtil.getDate() + " 23:59:59");
        }*/
    ...
    // 初审
    /*@Transactional(rollbackFor = Exception.class)
	public long updateBorrowStatus(int bid, int status, long adminId, String review,) throws Exception {
		long ret = 0;
		// 修改对应的借款状态
		Date now = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
...
*/
    @Transactional(rollbackFor = Exception.class)
    public long updateBorrowStatus(int bid, int benJin, int paidFee, int status, long adminId, String review) throws Exception {
        long ret = 0;
        // 修改对应的借款状态
        Date now = new Date();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Model m = new Model("borrow_main")
                .set("borrowStatus", status)
                .set("fristAuditTime", sdf.format(now))
                .set("benJin",benJin)
                .set("serviceFee",(benJin-paidFee))
                .set("totalFee",(benJin-paidFee));
        ret = m.update(bid);
        if (ret < 0) {
            return -1;
        }
    ...
    public long updateBorrowStatusByOrderNo(String orderNo, int borrowStatus) throws Exception {
        ...
        if (map != null) {
            ...
            Map<String, String> memberRelationMap = memberService.findMemberRelation(Convert.strToLong(map.get("member_id"), 0));
            if (memberRelationMap != null) {
                ...
                if (Convert.strToInt(memberRelationMap.get("status"), 0) == 0) {
                    ...
                    if ("0".equals(memberRelationMap.get("memberId"))) {
                        ...
                        //每个邀请的只能发放一次佣金
                        memberService.updateCommision(getMember, jiLiangNo, member, map, Convert.strToInt(memberRelationMap.get("id"), 0));
