...
public class LLPayUtil {
	...
	public static Log log = LogFactory.getLog(LLPayUtil.class); 
	/**
	 * str空判断
	 * 
	 * @param str
	...
	private static boolean checkSignRSA(JSONObject reqObj, String rsa_public) {
		...
		try {
			...
			log.error("商户[" + reqObj.getString("oid_partner") + "]RSA签名验证异常" + e.getMessage());
			return false;
		}
	}
	...
	private static boolean checkSignMD5(JSONObject reqObj, String md5_key) {
		...
		if (sign.equals(DigestUtils.md5Hex(sign_src))) {
			log.debug("商户[" + reqObj.getString("oid_partner") + "]MD5签名验证通过");
			return true;
		} else {
			log.error("商户[" + reqObj.getString("oid_partner") + "]MD5签名验证未通过");
			return false;
		}
	}

	public static boolean checkMD5Sign(String data, String md5_key,String sign) {
		// 生成待签名串
		String sign_src = data;
		sign_src += "&key=" + md5_key;
		log.info(DigestUtils.md5Hex(sign_src));
		return DigestUtils.md5Hex(sign_src).equalsIgnoreCase(sign);
	}
	...
	private static String addSignRSA(JSONObject reqObj, String rsa_private) {
		...
		try {
			String sign = TraderRSAUtil.sign(rsa_private, sign_src);
			log.debug("sign:"+sign);
			return sign;
		} catch (Exception e) {
			log.error("商户[" + reqObj.getString("oid_partner") + "]RSA加签名异常" + e.getMessage());
			return "";
		}
	}
	...
	private static String addSignMD5(JSONObject reqObj, String md5_key) {
		...
		try {
			...
			log.error("商户[" + reqObj.getString("oid_partner") + "]MD5加签名异常" + e.getMessage());
			return "";
		}
	}
